...
impl EnumGenerator {
    ...
    pub fn new(input: &mut syn::DeriveInput) -> syn::Result<Self> {
        ...
        match &mut input.data {
                ...
                for variant in data.variants.iter_mut() {
                    match &variant.fields {
                        syn::Fields::Unnamed(fields) => {
                            if fields.unnamed.len() != 1 {
                                return Err(syn::Error::new(
                                    variant.ident.span(),
                                    "`Debuggable`: variants with more than 1 unnamed field are not supported",
                                ));
                            }
                        }
                        ...
                        syn::Fields::Named(_) => {
                            return Err(syn::Error::new(
                                variant.ident.span(),
                                "`Debuggable`: variants with named fields are not supported",
                            ));
                        }
                    ...
                    if matches!(variant_attribute.value_as, As::Debuggable)
                    ...
                    if matches!(variant_attribute.key_as, As::Debuggable)
                        && !matches!(variant_attribute.key_style, Style::None)
                    {
                        return Err(syn::Error::new(
                            variant.span(),
            ...
            _ => return Err(syn::Error::new(input.ident.span(), "`Debuggable`: not an enum")),
        }
