        ...
        match self.hint {
            Some(address) => {
                if self.allow_unspecified || !address.is_unspecified() {
                    Ok(vec![ListenableAddress::new_with_ipv6(address, self.flowinfo, self.scope)])
                } else {
                    match address {
            ...
            None => {
                if self.allow_unspecified {
                    Ok(vec![
                        ListenableAddress::new_with_ipv6(Ipv6Addr::UNSPECIFIED.into(), None, None),
                        ListenableAddress::new_with_ipv6(Ipv4Addr::UNSPECIFIED.into(), None, None),
                    ])
                } else {
