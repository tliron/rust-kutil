...
impl HostRouter {
    ...
    /// Add.
    pub fn add(&mut self, host: ByteString, router: Router) {
    ...
    pub fn fallback_router(&mut self) -> Option<&mut Router> {
        match &self.fallback_host {
            Some(host) => self.routers.get_mut(host),
            None => None,
        }
    }

    /// Handle a [Request] by forwarding it to a router if possible.
    pub async fn handle(&mut self, host: ByteString, request: Request) -> Option<Response> {
        let router = match self.routers.get_mut(&host) {
            Some(router) => {
                tracing::debug!("router for host: {}", host);
                router
            }

            None => match self.fallback_router() {
                Some(router) => {
                    tracing::debug!("using fallback, no router for host: {}", host);
                    router
                }

                None => {
                    tracing::debug!("no fallback and no router for host: {}", host);
                    return None;
                }
            },
        };
...
pub async fn host_routers_handler(
    ...
    host_routers.handle(host.into(), request).await.unwrap_or_else(|| StatusCode::NOT_FOUND.into_response())
}
