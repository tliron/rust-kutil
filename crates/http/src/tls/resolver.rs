        ...
        match self {
            Self::BySNI(targets) => match client_hello.server_name() {
                Some(sni) => match targets.get(sni) {
                    Some(target) => target.resolve(client_hello),
                    None => None,
                },
            ...
            Self::Single(target) => target.resolve(client_hello),
        }
    }
}
...
impl ResolvesServerCert for SniResolverTarget {
    fn resolve(&self, client_hello: ClientHello<'_>) -> Option<Arc<rustls::sign::CertifiedKey>> {
        match self {
            Self::Key(certified_key) => Some(certified_key.clone()),
            Self::Delegate(delegate) => delegate.resolve(client_hello),
        }
    }
}
