...
use {
    ::bytes::*,
    kutil_std::{collections::*, error::*},
    kutil_transcoding::{bytes::*, *},
};
...
impl CachedBody {
    ...
    pub async fn new_with(
        ...
        if preferred_encoding == encoding {
            ...
            representations.insert(preferred_encoding.clone(), bytes.encode(&preferred_encoding).await?);
            if keep_identity_encoding {
            ...
            representations.insert(preferred_encoding, identity_bytes);
        } else {
            // Reencode (via Identity)
            tracing::debug!("reencoding from {} to {}", encoding, preferred_encoding);
            let identity_bytes = bytes.decode(&encoding).await?;
            representations.insert(preferred_encoding.clone(), identity_bytes.encode(&preferred_encoding).await?);
            if keep_identity_encoding {
    ...
    pub async fn get(&self, encoding: &Encoding, keep_identity: bool) -> Result<(Bytes, Option<Self>), CapturedError> {
