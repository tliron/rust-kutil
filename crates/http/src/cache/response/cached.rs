use super::{
    super::{super::headers::*, body::*, hooks::*, sized::*},
    pieces::*,
};
...
use {
    ...
    kutil_std::error::*,
    kutil_transcoding::*,
    std::{mem::*, result::Result, sync::*, time::*},
};
...
impl CachedResponse {
    ...
    pub async fn new_for<BodyT>(
        ...
        cache_duration: Option<(CacheDurationHook, &Uri)>,
    ) -> Result<Self, (CapturedError, Option<ResponsePieces<BodyT>>)>
    where
        BodyT: Body + Unpin,
        BodyT::Error: Into<CapturedError>,
    {
        ...
        let bytes = match body.read_into_bytes_bounded(known_body_size, min_body_size, max_body_size).await {
            Ok(bytes) => bytes,
            Err((error, body_pieces)) => {
                return Err((error, body_pieces.map(|pieces| ResponsePieces::new_from(parts, pieces))));
            }
        };

        if (preferred_encoding != Encoding::Identity) && !parts.headers.encode(encodable_by_default) {
            tracing::debug!("not encoding to {} (XX-Encode=false)", preferred_encoding);
        ...
        let duration = match parts.headers.cache_duration() {
            Some(duration) => Some(duration),
            None => match cache_duration {
                Some((cache_duration, uri)) => cache_duration(CacheDurationHookContext::new(uri, &parts.headers)),
                None => None,
            },
        };
        ...
        if !parts.headers.contains_key(LAST_MODIFIED) {
            parts.headers.set_value(LAST_MODIFIED, now());
        }

        parts.headers.remove(CACHE_HEADER);
        parts.headers.remove(CACHE_DURATION_HEADER);
        ...
        if skip_encoding {
            parts.headers.set_bool_value(ENCODE_HEADER, true);
        }
    ...
    pub async fn to_response<BodyT>(
        ...
        encodable_by_default: bool,
        keep_identity_encoding: bool,
    ) -> Result<(Response<BodyT>, Option<Self>), CapturedError>
    where
        BodyT: Body + From<Bytes>,
    {
        if (*encoding != Encoding::Identity) && !self.headers().encode(encodable_by_default) {
            tracing::debug!("not encoding to {} (XX-Encode=false)", encoding);
            encoding = &Encoding::Identity;
        }

        let (bytes, modified) = self.body.get(encoding, keep_identity_encoding).await?;

        let mut parts = self.parts.clone();

        parts.headers.remove(ENCODE_HEADER);

        if *encoding != Encoding::Identity {
            // No need to specify Identity as it's the default
            parts.headers.set_value(CONTENT_ENCODING, encoding.clone());
        }

        parts.headers.set_native_value(CONTENT_LENGTH, bytes.len());

        Ok((Response::from_parts(parts, bytes.into()), modified.map(|body| self.clone_with_body(body))))
    }
}

impl Sized for CachedResponse {
    fn size(&self) -> usize {
