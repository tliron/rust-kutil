...
impl<SelectionT> Parseable for Selector<SelectionT>
where
    SelectionT: Parseable,
{
    fn parse(representation: &str) -> Option<Self> {
        if representation == "*" { Some(Self::Any) } else { Some(Self::Specific(SelectionT::parse(representation)?)) }
    }
}

impl<SelectionT> Sized for Selector<SelectionT>
where
    SelectionT: Sized,
{
    fn size(&self) -> usize {
    ...
    SelectionT: fmt::Display,
{
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            Self::Any => "*".fmt(formatter),
            Self::Specific(selector) => selector.fmt(formatter),
        }
    }
}
