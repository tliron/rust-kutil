use super::{bool::*, encoding::*, etag::*, into::*, language::*, media_type::*, parseable::*, preferences::*};
...
pub trait ParseHeaders {
    ...
    fn from_str_value<FromStrT>(&self, name: HeaderName) -> Option<FromStrT>
    where
        FromStrT: FromStr,
    ...
    ///
    /// [None](Option::None) could mean that there is no such header *or* that it is malformed.
    fn parseable_value<ParseableT>(&self, name: HeaderName) -> Option<ParseableT>
    where
        ParseableT: Parseable,
    {
        ParseableT::parse(self.string_value(name)?)
    }
    ...
    fn set_native_value<ValueT>(&mut self, name: HeaderName, value: ValueT)
    where
    ...
    fn set_bool_value(&mut self, name: HeaderName, value: bool) {
        self.set_value(name, if value { TRUE_HEADER_VALUE } else { FALSE_HEADER_VALUE });
    }
    ...
    fn content_length(&self) -> Option<usize> {
        self.from_str_value(CONTENT_LENGTH)
    }
    ...
    fn content_type(&self) -> Option<MediaType> {
        self.parseable_value(CONTENT_TYPE)
    }
    ...
    fn if_none_match(&self) -> Option<ETagMatcher> {
        self.parseable_value(IF_NONE_MATCH)
    }
    ...
    ///
    /// Defaults to [Identity](Encoding::Identity) if there is no such header *or* that is
    /// malformed.
    fn content_encoding(&self) -> EncodingHeaderValue {
        self.parseable_value(CONTENT_ENCODING).unwrap_or_default()
    }
