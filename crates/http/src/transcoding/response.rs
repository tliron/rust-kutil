    ...
    BodyT::Error: Into<CapturedError>,
{
    ...
    fn with_passthrough_body(self) -> Response<TranscodingBody<BodyT>> {
        self.with_passthrough_body_with_first_bytes(None)
    }

    /// Into a [Response] with a passthrough [TranscodingBody].
    fn with_passthrough_body_with_first_bytes(self, first_bytes: Option<Bytes>) -> Response<TranscodingBody<BodyT>>;

    /// Into a [Response] with an encoding [TranscodingBody].
    fn with_encoding_body(self, encoding: &Encoding, encodable_by_default: bool) -> Response<TranscodingBody<BodyT>> {
        self.with_encoding_body_with_first_bytes(None, encoding, encodable_by_default)
    }

    /// Into a [Response] with an encoding [TranscodingBody].
    fn with_encoding_body_with_first_bytes(
        self,
    ...
    BodyT::Error: Into<CapturedError>,
{
    fn with_passthrough_body_with_first_bytes(self, first_bytes: Option<Bytes>) -> Response<TranscodingBody<BodyT>> {
        ...
        Response::from_parts(parts, body.into_passthrough_with_first_bytes(first_bytes))
    }
    ...
    fn with_encoding_body_with_first_bytes(
        ...
        if *encoding == Encoding::Identity {
            return self.with_passthrough_body_with_first_bytes(first_bytes);
        }

        let (mut parts, body) = self.into_parts();

        let encode = parts.headers.encode(encodable_by_default);
        parts.headers.remove(CACHE_HEADER);
        parts.headers.remove(ENCODE_HEADER);

        if !encode {
            tracing::debug!("not encoding to {} (XX-Encode=false)", encoding);
            return Response::from_parts(parts, body.into_passthrough_with_first_bytes(first_bytes));
        }
...
where
    ...
    BodyT::Error: Into<CapturedError>,
{
    let mut response = Response::new(Bytes::new().into()).with_passthrough_body_with_first_bytes(None);
