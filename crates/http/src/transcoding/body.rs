...
use {
    ...
    pin_project::*,
    std::{collections::*, pin::*, result::Result, task::*},
    tokio_util::io::*,
};
    ...
    BodyT::Error: Into<CapturedError>,
{
    fn from(bytes: Bytes) -> Self {
        ...
        body.into_passthrough_with_first_bytes(None)
    }
}
    ...
    InnerBodyT::Error: Into<CapturedError>,
{
    ...
    fn poll_frame(
        ...
        Poll::Ready(match ready!(poll_read_buf(projected_self.reader, context, projected_self.buffer)) {
            Ok(count) => {
                if count != 0 {
                    ...
                    match &mut self.trailers {
                        Some(trailers) => trailers.pop_front().map(|trailers| Ok(Frame::trailers(trailers))),
                        None => None,
                    }
                }
            }
    ...
    BodyT::Error: Into<CapturedError>,
{
    ...
    fn into_passthrough(self) -> TranscodingBody<BodyT> {
        self.into_passthrough_with_first_bytes(None)
    }

    /// Into passthrough [TranscodingBody].
    fn into_passthrough_with_first_bytes(self, first_bytes: Option<Bytes>) -> TranscodingBody<BodyT>;
    ...
    /// Into decoding [TranscodableBody].
