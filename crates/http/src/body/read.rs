...
pub trait ReadBodyIntoBytes: Sized {
    ...
    /// [ReadBodyError] with [io::ErrorKind::FileTooLarge] and [BodyPieces], the latter of which
    /// can be used by the caller to reconstruct the original body, e.g. with
    ...
    /// Read entire [Body] into [Bytes] and trailers.
    ///
    /// If we we did not read all the way to EOF will return a [io::ErrorKind::FileTooLarge] error.
    ...
    BodyT::Error: Into<CapturedError>,
{
    async fn read_into_bytes_or_pieces(
        ...
        reader.read_buf(&mut bytes).await.map_err(|error| ErrorWithBodyPieces::from(ReadBodyError::from(error)))?;

        // Did we read to completion?
                ...
                if error.kind() == io::ErrorKind::UnexpectedEof {
                    ...
                    if let Some(declared_size) = declared_size {
                        if declared_size != fulfilled_size {
                            ...
                            tracing::warn!(
                                "declared size is {} but actual body size is {}",
                                declared_size,
                                fulfilled_size
                            );
                        }
                    ...
                    let (body, remainder, _trailers) = reader.into_inner();
                    bytes.put(remainder); // remainder *should* be empty
                    Err(ErrorWithBodyPieces::new(error.into(), Some(BodyPieces::new(body, bytes.into()))))
                }
            }
        }
    }
