...
/// optimization that is impossible to achieve if the encoding and caching layers are implemented
/// as independent layers. Far too many web servers ignore this optimization and waste compute
...
/// [included examples](https://github.com/tliron/rust-kutil/tree/main/crates/http/examples)!
///
/// Requirements
/// ============
///
/// The response body type *and* its data type must both implement
/// [From]\<[Bytes]\>. (This is supported by [axum](https://github.com/tokio-rs/axum).) Note that
/// even though [Tokio](https://github.com/tokio-rs/tokio) I/O types are used internally, this
/// layer does *not* require a specific async runtime.
///
/// Usage notes
/// ===========
///
...
///    generally good practice for all HTTP components to run optimally. That said, this layer will
///    optimize as much as it can even when `Content-Length` is not available, reading only as many
///    bytes as necessary to determine if the response is cacheable and then "pushing back" those
///    bytes (zero-copy) if it decides to skip the cache and send the response downstream.
///
...
///       requests. If your cache is distributed it might mean syncing the cache first.
///
...
///       entries that depend on that data. To simplify this, you can create a custom key struct
///       that includes the data IDs. When invalidating, you can then enumerate all existing keys
///       that contain the relevant ID.
///
/// Request handling
/// ================
///
...
///
/// 1. A request arrives. Check if it is cacheable. Reasons it won't be cacheable:
///
...
///       priorities specified in the request's `Accept-Encoding`. If the response has `XX-Encode`
///       header as "false" then use Identity encoding. (We keep the header stored for step 3.1
///       above.)
///
///    3. If the encoding is not Identity then we give the
...
///
///    4. Read the response body into a buffer. If there is no `Content-Length` header then make
///       sure to read no more than our configured maximum size.
///
///    5. If there's still more data left or the data that was read is less than our configured
///       minimum size then it means the response is non-cacheable, so:
///
///       1. Push the data that we read back into the front of the response body.
///
///       2. Go to "Non-cached request handling" step 4 below.
///
///    6. Otherwise store the read bytes in the cache. We will also make sure to set the
///       `Last-Modified` header to the current time if the header wasn't already set. Note that
///       response trailers are discarded and *not* stored in the cache. (We make the assumption
///       that trailers are only relevant to "real" responses.) Go up to step 3.2.
///
/// ### Non-cached request handling
///
/// 1. If the response has `XX-Encode` header as "false" then remove that header and pass through
///    the response. END.
///
...
///
/// 4. If the response is already in the selected encoding then pass it through. END.
///
/// 5. Otherwise, if the response is in a different encoding, and that encoding is *not* Identity,
///    then pass it through. END.
///
///    Note that this is technically wrong and in fact there is no guarantee here that the client
///    would support the encoding. However, we implement it this way because:
///
...
///
/// 6. Otherwise, wrap the response body with an encoder and send it. Note that we do not know the
///    size in advance so we make sure there is no `Content-Length` header. END.
    ...
    CacheKeyT: CacheKey,
{
    ...
    // /// Provide a hook to generate a [CacheKey].
    // ///
    // /// The headers are *request* headers.
    // ///
    // /// A common practice is to call the [default](CacheKey::default) hook and then set custom
    // /// [properties](CacheKey::properties) to further differentiate requests. However, it's also
    // /// possible to provide an entirely different way to generate cache keys.
    // ///
    // /// The default is [CacheKey::default].
    // pub fn cache_key(
    //     mut self,
    //     cache_key: impl Fn(&Method, &Uri, &HeaderMap) -> CacheKey + 'static + Send + Sync,
    // ) -> Self {
    //     self.caching.cache_key = Arc::new(Box::new(cache_key));
    //     self
    // }
    ...
    pub fn min_body_size(mut self, min_body_size: usize) -> Self {
        self.caching.min_body_size = min_body_size;
        self
    }
    ...
    pub fn max_body_size(mut self, max_body_size: usize) -> Self {
        self.caching.max_body_size = max_body_size;
        self
    }
    ...
    pub fn cacheable_by_default(mut self, cacheable_by_default: bool) -> Self {
        self.caching.cacheable_by_default = cacheable_by_default;
        self
    }
    ...
    pub fn cache_duration(
        ...
        self.caching.cache_duration = Some(Arc::new(Box::new(cache_duration)));
        self
    }
    ...
    /// select the best.
    ///
    /// There is no need to specify [Identity](Encoding::Identity) as it is always enabled.
    ///
    ...
    pub fn encodable_by_default(mut self, encodable_by_default: bool) -> Self {
        self.encoding.encodable_by_default = encodable_by_default;
        self
    }
    ...
    /// Whether to keep an [Identity](Encoding::Identity) in the cache if it is created during
    /// reencoding.
    ///
    ...
    pub fn keep_identity_encoding(mut self, keep_identity_encoding: bool) -> Self {
        self.encoding.keep_identity_encoding = keep_identity_encoding;
        self
    }
}
    ...
    CacheKeyT: CacheKey,
{
    ...
    fn layer(&self, inner: InnerServiceT) -> Self::Service {
        CachingService::new(inner, self.caching.clone(), self.encoding.clone())
    }
}
