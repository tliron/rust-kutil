...
pub struct Generator {
    ...
    pub enum_attribute: DisplayEnumAttribute,
...
impl Generator {
    ...
    pub fn generate(input: &mut syn::DeriveInput) -> syn::Result<TokenStream> {
        ...
        let stream = generator.generate_impl_display();
        Ok(stream)
    }
    ...
    pub fn new(input: &mut syn::DeriveInput) -> syn::Result<Self> {
        ...
        match &mut input.data {
                ...
                for variant in data.variants.iter_mut() {
                    if !variant.fields.is_empty() {
                        return Err(syn::Error::new(
                            variant.ident.span(),
                            "`Display`: variants with fields are not supported",
                        ));
                    }
            ...
            _ => return Err(syn::Error::new(input.ident.span(), "`Display`: not an enum")),
        }
